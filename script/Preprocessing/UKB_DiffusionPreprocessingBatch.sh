...
get_batch_options() {
    ...
    command_line_specified_no_gpu="FALSE"
    ...
    while [ ${index} -lt ${numArgs} ]; do
        ...
        case ${argument} in
            ...
            --runlocal)
                ...
                ;;
            --no-gpu)
                command_line_specified_no_gpu="TRUE"
                index=$(( index + 1 ))
                ;;
            *)
                echo ""
...
EnvironmentScript="tools/HCP_Pipelines/HCPpipelines-4.7.0/Examples/Scripts/SetUpHCPPipeline.sh" # Pipeline environment script
...
for Subject in $Subjlist ; do
  ...
  PosData="${RawDataDir}/${SubjectID}_3T_DWI_dir105_PA.nii.gz"
  NegData="${RawDataDir}/${SubjectID}_3T_DWI_dir105_AP.nii.gz"
  ...
  if [[ "${command_line_specified_run_local}" == "TRUE" || "$QUEUE" == "" ]] ; then
      echo "About to run ${HCPPIPEDIR}/DiffusionPreprocessing/DiffPreprocPipelineProtocol1.sh"
      queuing_command=("${FSLDIR}/bin/fsl_sub")
  else
      echo "About to use fsl_sub to queue ${HCPPIPEDIR}/DiffusionPreprocessing/DiffPreprocPipelineProtocol1.sh"
      queuing_command=("${FSLDIR}/bin/fsl_sub" -q "$QUEUE")
  fi

  if [[ "${command_line_specified_no_gpu}" == "TRUE" ]]; then
      no_gpu_flag="--no-gpu"
  else
      no_gpu_flag=""
  fi

  "${queuing_command[@]}" "${HCPPIPEDIR}"/DiffusionPreprocessing/DiffPreprocPipelineProtocol1.sh \
